...
function createMascotStateStore() {
  ...
  const _state = writable(
    createMascotState({ current: ANIMATION_STATES.INITIAL }),
  );
  ...
  function setAnimationState(newState) {
    ...
    _state.update((s) => ({
      ...
      current: newState,
      specialAnimation:
        newState === ANIMATION_STATES.EASTER_EGG ? s.specialAnimation : null,
    ...
    if (behavior.cleanupDelay && behavior.cleanupDelay > 0) {
      debugLog(
        `Setting cleanup timeout for ${newState} → IDLE in ${behavior.cleanupDelay}ms`,
      );
  ...
  function applyWobbleAnimation(type) {
    const spinPivot = wobbleTargetElement?.isConnected
      ? wobbleTargetElement
      : null;
    if (!spinPivot) {
      debugLog(
        `[Wobble] Could not find .mascot-spin-pivot target for ${type}`,
        "warn",
      );
      return;
    }
  ...
  function handleRecordingStart(currentState) {
    ...
    if (currentState.stateTimeouts[RAF_RECORDING_START_TIMEOUT]) {
      cancelAnimationFrame(
        currentState.stateTimeouts[RAF_RECORDING_START_TIMEOUT],
      );
    }
  ...
  const _specialAnimation = derived(
    _state,
    ($state) => $state.specialAnimation,
  );
  // ---
