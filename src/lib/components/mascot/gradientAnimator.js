...
export function initGradientAnimation(themeId, svgElement) {
  ...
  const gradient =
    svgElement.querySelector(`#${gradientId}`) ||
...
function initGradientPositionAnimation(themeId, gradient) {
  ...
  const amplitudeFromCss = parseFloat(
    getCssVariable(
      `mascot-${themeId}-position-amplitude`,
      config.position.amplitude,
    ),
  );
...
function initStopColorAnimations(themeId, stops) {
  ...
  const positions = config.stopPositions || [
    "start",
    "mid1",
    "mid2",
    "mid3",
  ...
  stops.forEach((stop, index) => {
    ...
    const opacityRangeKey =
      index === 0 ? "start" : index === stops.length - 1 ? "end" : "mid";
...
function animateStopColor(themeId, stopIndex) {
  ...
  if (factor > 0.5) {
    ...
    colorState.stop.setAttribute(
      "stop-color",
      getThemeColor(themeId, colorState.position, true),
    );
  } else {
    // Use base color when factor is low
    colorState.stop.setAttribute(
      "stop-color",
      getThemeColor(themeId, colorState.position, false),
    );
  }
